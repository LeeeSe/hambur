...
use super::{
    AppBootstrap, BackgroundProcessSession, DelegateTaskState, ModelRouteSnapshot, NewTraceSpan,
    ProcessOutputBuffer, RouteStreamSource, RuntimeCommand, RuntimeEngine, RuntimeEvent,
    platform_shell, provider_stream_source, spawn_process_pipe_reader, validate_web_fetch_url,
};
...
fn bundled_skills_are_seeded_and_exposed_by_tools() {
    ...
    let list = runtime
        .tokio
        .block_on(runtime.resolve_knowledge_tool_result(&list_invocation, &serde_json::json!({})))
        .content_json;
    ...
    let alias = runtime
        .tokio
        .block_on(runtime.resolve_knowledge_tool_result(&alias_invocation, &serde_json::json!({})));
...
fn scripted_stream_persists_reasoning_content_and_model_snapshot() {
    ...
    let ack = runtime.dispatch(RuntimeCommand {
        ...
        content: "hello".to_string(),
        payload_json: r#"{"content":"hello world","reasoning":"thinking separately"}"#.to_string(),
        ..RuntimeCommand::default()
    });
...
#[test]
fn attachment_hint_is_sent_to_model_but_not_persisted_in_user_message() {
    let listener = TcpListener::bind("127.0.0.1:0").expect("bind provider server");
    let addr = listener.local_addr().expect("local addr");
    let captured_request = Arc::new(Mutex::new(String::new()));
    let captured = captured_request.clone();
    let server = thread::spawn(move || {
        if let Ok((mut stream, _)) = listener.accept() {
            ...
            *captured.lock().expect("capture request") = text;
            let body = concat!(
                "data: {\"choices\":[{\"delta\":{\"content\":\"attachment seen\"}}]}\n\n",
                "data: [DONE]\n\n"
            );
            let response = format!(
                "HTTP/1.1 200 OK\r\nContent-Type: text/event-stream\r\nTransfer-Encoding: chunked\r\nConnection: close\r\n\r\n{:X}\r\n{}\r\n0\r\n\r\n",
    ...
    configure_http_test_provider(
        &runtime,
        "provider_attachment_hint",
        "gpt-attachment",
        &format!("http://{addr}/v1"),
    );

    let import = runtime.dispatch(RuntimeCommand {
        command_id: "cmd_import_http_image".to_string(),
        idempotency_key: "content://image:http:test".to_string(),
    ...
    let send = runtime.dispatch(RuntimeCommand {
        command_id: "cmd_send_http_image".to_string(),
        idempotency_key: "message:image:http".to_string(),
        kind: "SendMessage".to_string(),
        session_id: session_id.clone(),
        content: "crop this image".to_string(),
    ...
    assert_eq!(user_message.content_text, "crop this image");
    assert!(!user_message.content_text.contains("用户附加了一张图片"));
    ...
    let user_context = messages
        ...
        .expect("user model context");
    assert!(user_context.contains("crop this image"));
    assert!(user_context.contains("用户附加了一张图片"));
    assert!(user_context.contains("view_image"));

    let _ = fs::remove_dir_all(app_files_dir);
}

#[test]
fn attachment_without_text_is_rejected_and_remains_pending() {
    let app_files_dir = temp_app_dir();
    fs::create_dir_all(&app_files_dir).expect("create temp app dir");
    let runtime = RuntimeEngine::create(AppBootstrap {
        app_files_dir: app_files_dir.to_string_lossy().to_string(),
    ...
    let import = runtime.dispatch(RuntimeCommand {
        command_id: "cmd_import_only_attachment".to_string(),
        idempotency_key: "content://only-attachment:test".to_string(),
        kind: "ImportAttachmentFromUri".to_string(),
        session_id: session_id.clone(),
        payload_json: serde_json::json!({
            "displayName": "photo.png",
    ...
    let send = runtime.dispatch(RuntimeCommand {
        command_id: "cmd_send_only_attachment".to_string(),
        idempotency_key: "message:only-attachment".to_string(),
        kind: "SendMessage".to_string(),
        session_id: session_id.clone(),
        content: String::new(),
        payload_json: serde_json::json!({"attachmentIds": [attachment_id]}).to_string(),
    ...
    assert_eq!(send.rejection_code, "InvalidCommand");
    let snapshot = runtime.get_session_snapshot(session_id);
    assert_eq!(snapshot.pending_attachments.len(), 1);
    assert_eq!(snapshot.pending_attachments[0].display_name, "photo.png");
...
fn duplicate_send_message_is_rejected_while_turn_is_active() {
    ...
    let first = runtime.dispatch(RuntimeCommand {
        ...
        payload_json: r#"{"content":"one two three four five six","reasoning":"busy"}"#.to_string(),
        ..RuntimeCommand::default()
    });
    ...
    assert!(
        messages.iter().any(|message| {
            message.get("role").and_then(Value::as_str) == Some("tool")
                && message.get("tool_call_id").and_then(Value::as_str) == Some("call_echo_history")
                && message
                    .get("content")
...
fn attachment_import_remove_and_startup_cleanup() {
    ...
    assert_eq!(imported.snapshot.pending_attachments[0].kind, "image");
    let attachment_id = imported.snapshot.pending_attachments[0].id.clone();
...
fn send_message_consumes_image_attachment_and_requires_vision_route() {
    ...
    let user_message = runtime
        ...
        .message
        .expect("user message snapshot");
    assert_eq!(user_message.content_text, "describe this");
    assert!(!user_message.content_text.contains("用户附加了一张图片"));
    assert!(
        runtime
...
#[test]
fn text_only_route_accepts_image_attachment_as_file_reference() {
    let app_files_dir = temp_app_dir();
    fs::create_dir_all(&app_files_dir).expect("create temp app dir");
    ...
    let send = runtime.dispatch(RuntimeCommand {
        ...
        payload_json: serde_json::json!({
            "attachmentIds": [attachment_id],
            "content": "text route accepted image attachment as file reference"
        })
        .to_string(),
        ..RuntimeCommand::default()
    });
    assert!(send.accepted, "send rejected: {}", send.message);
    wait_for_session_finished(&runtime, &session_id, "text route image attachment");
    let timeline = runtime.get_timeline_page(session_id.clone(), 0, 20);
    let user = timeline
        .items
        .iter()
...
fn view_image_text_route_hands_off_to_vision_continuation() {
    ...
    let send = runtime.dispatch(RuntimeCommand {
        ...
        payload_json: serde_json::json!({"sse": sse, "sse_sequence": [continuation_sse]})
            .to_string(),
        ..RuntimeCommand::default()
    });
    assert!(send.accepted, "send rejected: {}", send.message);
...
fn milestone6_global_settings_commands_match_architecture_contract() {
    ...
    let theme = runtime.dispatch(RuntimeCommand {
        ...
        payload_json: serde_json::json!({"settingKey": "themeMode", "value": "dark"}).to_string(),
        ..RuntimeCommand::default()
    });
    ...
    assert!(
        snapshot
            ...
            .settings
            .iter()
            .any(|setting| setting.key == "skill_enabled:skills-test" && setting.value == "false")
    );
    assert!(
        snapshot
            .settings
...
fn milestone7_submit_delegate_result_copies_child_artifacts_to_parent_workspace() {
    ...
    let parent_path = format!("/var/hambur/workspace/delegates/{delegate_session_id}/report.txt");
...
fn milestone7_process_wait_keeps_finished_log_snapshot() {
    ...
    let result =
        runtime.resolve_process_tool_result(&invocation, &invocation.arguments_value().unwrap());
    ...
    let log_result = runtime
        .resolve_process_tool_result(&log_invocation, &log_invocation.arguments_value().unwrap());
    assert!(
        !log_result.is_error,
        "process log failed: {}",
        log_result.summary
...
fn first_user_message_titles_default_session() {
    ...
    assert_eq!(
        session.title,
        "请帮我解释 Kotlin Flow 的背压问题，以及怎么处理"
    );
...
fn latest_assistant_message_snapshot(runtime: &RuntimeEngine, session_id: &str) -> MessageRecord {
    runtime
        .tokio
...
fn configure_fallback_group(runtime: &RuntimeEngine, group_id: &str, targets: &[(&str, &str)]) {
    let group = runtime.dispatch(RuntimeCommand {
...
#[test]
fn refresh_provider_models_enriches_from_cached_models_dev_catalog() {
    let app_files_dir = temp_app_dir();
    fs::create_dir_all(&app_files_dir).expect("create temp app dir");
    ...
    runtime
        .tokio
        .block_on(
            runtime.database.upsert_model_catalog_cache(
    ...
    let provider = runtime.dispatch(RuntimeCommand {
        command_id: "cmd_catalog_provider".to_string(),
        idempotency_key: "catalog-provider:update:test".to_string(),
        kind: "UpdateProvider".to_string(),
        provider_id: "provider_catalog".to_string(),
        title: "Catalog Provider".to_string(),
        chunk: "https://api.test/v1".to_string(),
    ...
    let models = runtime.dispatch(RuntimeCommand {
        command_id: "cmd_catalog_models".to_string(),
        idempotency_key: "catalog-provider:models:test".to_string(),
        kind: "RefreshProviderModels".to_string(),
        provider_id: "provider_catalog".to_string(),
        payload_json: serde_json::json!({
            "data": [{
    ...
    let model = runtime
        ...
        .block_on(
            runtime
                .database
                .provider_model_by_key("provider_catalog", "gpt-4o-mini"),
        )
    ...
    assert_eq!(model.context_limit, 128000);
    assert_eq!(model.output_limit, 16384);
    assert_eq!(model.reasoning_field, "reasoning_content");
    assert!(model.metadata_json.contains("\"cost\""));
    assert!(model.metadata_json.contains("\"family\":\"gpt-4o\""));
}
