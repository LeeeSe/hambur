...
#[cfg(test)]
mod tests {
    use super::{MarkdownPipeline, render_markdown_to_nodes};

    #[test]
    fn different_chunk_sizes_produce_same_final_nodes() {
        let markdown = "# Title\n\nHello **world**.\n\n```rust\nfn main() {}\n```\n\n| A | B |\n| - | - |\n| 1 | 2 |\n";
        let whole = render_markdown_to_nodes("msg", markdown);

        let mut streamed = MarkdownPipeline::new("msg");
        let mut streamed_nodes = Vec::new();
        for chunk in markdown.as_bytes().chunks(5) {
            let chunk = std::str::from_utf8(chunk).expect("utf8 chunk");
            streamed_nodes.extend(streamed.append(chunk).committed_nodes);
        }
        streamed_nodes.extend(streamed.finalize().committed_nodes);

        assert_eq!(streamed_nodes, whole);
    }

    #[test]
    fn pending_updates_do_not_reemit_committed_blocks() {
        let mut pipeline = MarkdownPipeline::new("msg");
        let first = pipeline.append("Stable paragraph.\n\npend");
        assert_eq!(first.committed_nodes.len(), 1);
        assert!(first.pending_node.is_some());

        let second = pipeline.append("ing");
        assert!(second.committed_nodes.is_empty());
        assert_eq!(second.pending_node.expect("pending").block_id, 2);
    }

    #[test]
    fn pending_code_fence_uses_display_repair() {
        let mut pipeline = MarkdownPipeline::new("msg");
        let update = pipeline.append("```rust\nfn main()");
        let pending = update.pending_node.expect("pending code fence");

        assert_eq!(pending.node_kind, "CodeBlock");
        assert_eq!(pending.language, "rust");
        assert_eq!(pending.text, "fn main()");
    }

    #[test]
    fn table_and_hambur_file_links_are_semantic_nodes() {
        let table = render_markdown_to_nodes("msg", "| A | B |\n| - | - |\n| 1 | 2 |\n");
        assert_eq!(table[0].node_kind, "Table");
        assert_eq!(table[0].table_header, vec!["A", "B"]);

        let file = render_markdown_to_nodes("msg", "[report](hambur://file/report.pdf)\n");
        assert_eq!(file[0].node_kind, "HamburFileBlock");
        assert_eq!(file[0].file_kind, "file");
    }

    #[test]
    fn pending_links_do_not_break_and_commit_to_file_nodes() {
        let mut pipeline = MarkdownPipeline::new("msg");
        let pending = pipeline.append("[report](hambur://file/report");
        assert!(pending.pending_node.is_some());
        assert!(pending.committed_nodes.is_empty());

        let second = pipeline.append(".md)\n\n");
        assert!(second.pending_node.is_some() || !second.committed_nodes.is_empty());

        let finalized = pipeline.finalize();
        let committed = second
            .committed_nodes
            .into_iter()
            .chain(finalized.committed_nodes)
            .collect::<Vec<_>>();
        assert_eq!(committed.len(), 1);
        assert_eq!(committed[0].node_kind, "HamburFileBlock");
        assert_eq!(committed[0].path, "hambur://file/report.md");
    }

    #[test]
    fn lists_and_blockquotes_serialize_children_as_json() {
        let list = render_markdown_to_nodes("msg", "- one\n- **two**\n");
        assert_eq!(list[0].node_kind, "List");
        assert!(list[0].items_json.starts_with('['));
        assert!(list[0].items_json.contains("Paragraph"));

        let quote = render_markdown_to_nodes("msg", "> quoted **text**\n");
        assert_eq!(quote[0].node_kind, "BlockQuote");
        assert!(quote[0].children_json.starts_with('['));
        assert!(quote[0].children_json.contains("Paragraph"));
    }
}
